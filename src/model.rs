...
impl Model {
    pub fn load(
            ...
            let meshes = obj_models
                ...
                .map(|m| {
                    ...
                    for c in indices.chunks(3) {
                        ...
                        let pos0 = cgmath::Vector3::<_>::from(v0.position);
                        let pos1 = cgmath::Vector3::<_>::from(v1.position);
                        let pos2 = cgmath::Vector3::<_>::from(v2.position);

                        let uv0 = cgmath::Vector2::<_>::from(v0.tex_coords);
                        let uv1 = cgmath::Vector2::<_>::from(v1.tex_coords);
                        let uv2 = cgmath::Vector2::<_>::from(v2.tex_coords);

                        // Calculate the edges of the triangle
                        let delta_pos1 = pos1 - pos0;
                        let delta_pos2 = pos2 - pos0;

                        // This will giveus a direction to calculate the tangent and bitangent
                        let delta_uv1 = uv1 - uv0;
                        let delta_uv2 = uv2 - uv0;

                        // Solving the following system of equations will
                        // give us the tangetn and bitangetn.
                        //     delta_pos1 = delta_uv1.x * T + delt_uv1.y * B
                        //     delta_pos2 = delta_uv2.x * T + delt_uv2.y * B
                        let r = 1.0 / (delta_uv1.x * delta_uv2.y - delta_uv1.y * delta_uv2.x);
                        let tangent = (delta_pos1 * delta_uv2.y - delta_pos2 * delta_uv1.y) * r;
                        let bitangent = (delta_pos2 * delta_uv1.y - delta_pos1 * delta_uv2.y) * r;

                        // We'll use the same tangent/bitangent for each vertex in the triangle
                        for &i in c {
                            vertices[i as usize].tangent = (tangent
                                + cgmath::Vector3::from(vertices[i as usize].tangent))
                            .into();
                            vertices[i as usize].bitangent = (bitangent
                                + cgmath::Vector3::from(vertices[i as usize].bitangent))
                            .into();
        ...
        inner(device, queue, layout, path.as_ref())
    }
}
